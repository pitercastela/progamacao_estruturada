...
def novo_monstro():
    ...
    apareceu!".
    """
    monstro = {"força": random.randint(5,25),
...
def iniciar_combate(aventureiro, monstro):
    ...
    while True:
        ...
        print(f"dano do aventureiro:{dano_aventureiro}")
        print(f"vida do monstro:{monstro["vida"]}")
        if monstro["vida"] <= 0:
            return True
        dano_monstro = monstro_atacar(monstro)
        aventureiro_defender(aventureiro, dano_monstro)
        print(f"dano do monstro:{dano_monstro}")
        print(f"vida do aventureiro:{aventureiro["vida"]}")
